 ...
 */
typedef struct Map {
    RadixTree radix; ///< Drzewo radix przechowujące wierzchołki miasta i krawędzie między nimi
    long long int nextId; ///< Unikalne ID, które można nadać kolejnemu wierzchołkowi w drzewie radix
    long long int dijkstraCalls; ///< Ilość wywołań algorytmu dijkstry w danej mapie
    Node *routeBegin[1000]; ///< Tablica początkowych miast dróg krajowych
    Node *routeEnd[1000]; ///< Tablica końcowych miast dróg krajowych
} Map;
...
/** @brief Tworzy drogę krajową według wczytanego opisu.
 * Wczytuje kolejne argumenty podane w stylu napisu zwracanego przez 
 * getRouteDescription i tworzy drogę krajową numer @p routeId w mapie @p map.
 * Ustawia @p lastChar na ostatni wczytany znak.
 * @param[in,out] map       – modyfikowana mapa;
 * @param[in] routeId       – numer tworzonej drogi krajowej;
 * @param[out] lastChar     – ostatni wczytany znak.
 */
bool createRoute(Map *map, int routeId, int *lastChar);

/** @brief Usuwa drogę krajową.
 * Usuwa drogę krajową o numerze @p routeId w mapie @p map bez usuwania odcinków
 * dróg, ani miast.
 * @param[in,out] map        – modyfikowana mapa;
 * @param[in] routeId        – numer usuwanej drogi krajowej.
 */
